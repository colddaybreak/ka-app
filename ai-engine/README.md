...
│   ├── models/
│   │   ├── embedding.py         # 向量模型（抽象基类 + OpenAI 实现）
│   │   └── llm.py               # 大语言模型工厂函数
│   ├── vectorstore/
...
| 向量化 | `models/embedding.py` | 调用 OpenAI Embedding API，将文本块转换为 1536 维向量 |
...
| 环节 | 说明 |
|------|------|
| 检索 | 取相似度最高且超过阈值（默认 0.7）的 5 个分块作为参考资料 |
...
| `OPENAI_API_KEY` | 无 | 必填，OpenAI API 密钥 |
...
│   ├── models/
│   │   ├── embedding.py         # Embedding models (abstract base + OpenAI impl)
...
| Embedding | `models/embedding.py` | Calls the OpenAI Embedding API to convert chunks into 1536-dim vectors |
...
| Stage | Description |
|-------|-------------|
| Retrieval | Takes the top 5 chunks above the similarity threshold (default 0.7) as references |
...
| `OPENAI_API_KEY` | none | Required; the OpenAI API key |
